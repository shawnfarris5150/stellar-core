...
Config::load(std::string const& filename)
{
    ...
    try
    {
        ...
        for (auto& item : g)
        {
            ...
            {
                FAILURE_SAFETY = readInt<int32_t>(item, 0, INT32_MAX - 1);
            }
            ...
            {
                TARGET_PEER_CONNECTIONS = readInt<unsigned short>(item);
            }
            else if (item.first == "MAX_PEER_CONNECTIONS")
            {
                MAX_PEER_CONNECTIONS = readInt<unsigned short>(item);
            }
            ...
            {
                MAX_CONCURRENT_SUBPROCESSES = readInt<size_t>(item);
            }
